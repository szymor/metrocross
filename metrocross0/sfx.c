...
//printf("%d %d\n", len, SFX_SAMPLES_CH * sizeof(pYmBuffer[0]));
...
//		nOutput = 0;
		...
		for (k = 0; k < SFX_MAX_SOUNDS; k++)
		{
			...
			{
				nOutput += (*(s16 *)&gpSounds[k].pData[gpSounds[k].nDPos]) / 8;		// / 2 pour baisser le son des fx.		// / 4;//2;		// / 2 pour réduire la saturation.
				gpSounds[k].nDPos += 2;
			}
		}
//		nOutput /= 4;//2;
...
void Sfx_SoundInit(void)
{
	...
	// Set 16-bit mono audio at 44Khz.	//// Set 16-bit stereo audio at 22Khz.
...
//printf("samples = %d\n", gSfx.sAudioSpec.samples * gSfx.sAudioSpec.channels * sizeof(s16));
	// Test de la taille des samples.
	if (gSfx.sAudioSpec.samples * gSfx.sAudioSpec.channels * sizeof(s16) >
		SFX_SAMPLES_CH * sizeof(s16))	// Taille du buffer : static s16 pYmBuffer[SFX_SAMPLES_CH];
	{
		fprintf(stderr, "Sfx_SoundInit(): Error. Sample sz (%d) > YmBuffer sz (%d).\n",
			(int)(gSfx.sAudioSpec.samples * gSfx.sAudioSpec.channels * sizeof(s16)),
			(int)(SFX_SAMPLES_CH * sizeof(s16)));
		exit(1);
	}
...
void Sfx_LoadWavFiles(void)
{
	...
	char	*pSfxFilenames[e_Sfx_LAST] = {
		"sfx/jump.wav",
		"sfx/flattened.wav",
		"sfx/spring_platform.wav",
		"sfx/exploding_panel.wav",
		"sfx/countdown.wav",
		"sfx/go.wav",
		"sfx/_can_crush2.wav",
		"sfx/_can_kick.wav",
		"sfx/_hurdle_hit.wav",
		"sfx/_tire_bounce.wav",
	};
	...
	for (i = 0; i < e_Sfx_LAST; i++)
	{
		...
		if (SDL_BuildAudioCVT(&gSfx.pCvt[i], sWave.format, sWave.channels, sWave.freq,
			gSfx.sAudioSpec.format, gSfx.sAudioSpec.channels, gSfx.sAudioSpec.freq) == 0)
		{
			fprintf(stderr, "Sfx_LoadWavFiles(): SDL_BuildAudioCVT failed (gSfx.pCvt[%d].buf).\n", (int)i);
			exit(1);
		}
...
void Sfx_LoadYMFiles(void)
{
	...
	static	char *pMusicFn[e_YmMusic_MAX] = {
		"sfx/mus_metrocross1.ym",		// e_YmMusic_GameOver
		"sfx/mus_metrocross2.ym",		// e_YmMusic_RoundOver
		"sfx/mus_metrocross3.ym",		// e_YmMusic_HighScore
		"sfx/mus_metrocross4.ym",		// e_YmMusic_MainTheme
		"sfx/mus_metrocross5.ym",		// e_YmMusic_ReadyYourself
	};
	// Loop ou non.
	static	ymbool pMusicLoop[e_YmMusic_MAX] = {
		YMFALSE,		// e_YmMusic_GameOver
		YMTRUE,			// e_YmMusic_RoundOver
		YMTRUE,			// e_YmMusic_HighScore
		YMTRUE,			// e_YmMusic_MainTheme
		YMFALSE,		// e_YmMusic_ReadyYourself
	};
