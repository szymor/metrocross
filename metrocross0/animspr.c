...
{
	SLOT_ERR(Anm_GetKey, e_AnmKey_Null)
	return (pAnmSlots[nSlotNo].nKey);
}

// Change une clef d'anim (metro-cross). !!! DANGER, ne pas utiliser n'importe comment !!!
u32 Anm_SetKey(s32 nSlotNo, u32 nKey)
{
	SLOT_ERR(Anm_GetKey, 0)
	pAnmSlots[nSlotNo].nKey = nKey;
	return (0);		// return juste pour ne pas faire une autre macro.
}
...
{
	SLOT_ERR(Anm_CheckNewImgFlag, 0)
...
s32 Anm_SetIfNew(u64 *pAnm, s32 nSlotNo)
{
	...
	if (nSlotNo == -1) return (Anm_Set(pAnm, nSlotNo));

	SLOT_ERR(Anm_SetIfNew, -1)	// APRES le test de -1 qui ici est valide. Par contre on teste si la valeur est au delà de la table.
...
s32 Anm_Set(u64 *pAnm, s32 nSlotNo)
{
	...
	if (nSlotNo == -1)
	{
		if ((nSlotNo = Anm_GetSlot()) == -1) //return (-1);
		{
			fprintf(stderr, "Anm_Set(): Warning, out of animation slots!\n");
			return (-1);
		}
		pAnmSlots[nSlotNo].nUsed = 1;
	}
	pAnmSlots[nSlotNo].nFlags = e_AnmFlag_Init;			// RAZ Flags. (Le flag 'init' est testé lors du premier GetImage).
...
//	SSprite	*pSpr;
	...
	SLOT_ERR(Anm_GetImage, SPR_NoSprite)
...
//todo: revoir si on peut ne pas faire l'opération à chaque fois \/.
	...
	if (--pAnmSlots[nSlotNo].nFramesCnt == 0)
	{
		...
		while (((*pAnmSlots[nSlotNo].pAnm) & BIT31) != 0)
		{
			...
			default:
				fprintf(stderr, "Anm: Unknown control code.\n");
				break;
			}
...
{
	SLOT_ERR(Anm_GetLastImage, SPR_NoSprite)
	return (pAnmSlots[nSlotNo].nCurSpr);	// N° du sprite en cours.
}
